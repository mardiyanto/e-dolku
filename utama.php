...
elseif($aksi=='kategori'){
  ...
  echo "
  <h2 >Kategori: $n[nama_kategori]</h2>";
  $p      = new Paging3;
  $batas  = 16;
...
elseif($aksi=='allproduk'){
   	  echo "
	  <h2 class='nama'><span>Semua Produk</span></h2>
...
if ((isset($_POST['submit'])) and ($_POST['cari']<>"")){
 ...
 $p      = new Paging;
  $batas  = 24;
  $posisi = $p->cariPosisi($batas);
$s=mysql_query("SELECT * FROM produk  WHERE nama_produk LIKE '%$cari%' or deskripsi LIKE '%$cari%' ORDER BY id_produk DESC LIMIT $posisi,$batas");
$jmldata     = mysql_num_rows(mysql_query("SELECT * FROM produk WHERE nama_produk LIKE '%$cari%' or deskripsi LIKE '%$cari%'"));
$jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
$linkHalaman = $p->navHalaman($_GET[halaman], $jmlhalaman);
$ada=mysql_numrows($s);
$no=1;
echo "<div class='card border-ligh'> 
    <h5 class='card-title'>Di temukan Ada $ada data</h5>
</div>";
if($ada > 0){
...
elseif($aksi=='diskon'){
   	  echo "
	  <h2 class='nama'>Semua Produk Diskon</h2>
	  ";
  $p      = new Paging;
  $batas  = 52;
  $posisi = $p->cariPosisi($batas);
...
$profil = mysql_query("SELECT * FROM profil WHERE id_profil='$_GET[id_p]' ");
  ...
  echo "<div class='card border-ligh'>
    ";
  if($r[aktif]=='Y'){echo"<img class='card-img-top' src='foto/foto_profil/$r[foto]' alt='$r[nama]'>";}
         echo"	
  <div class='card-body '>
...
<div class='form-title'>Deskripsi Toko</div>
	 <input type='text' name='deskripsi' placeholder='Email Anda' class='form-field' required>
<br />
<div class='form-title'>Alamat Toko</div>
	 <input type='text' name='alamat_toko' placeholder='Email Anda' class='form-field' required>
<br />
<div class='form-title'>Nama Bank</div>
 <input name='nama_bank'  placeholder='nama bank' type='text' class='form-field' required >
<br />

<div class='form-title'>Nomor Rekening</div>
 <input name='nomo_rekening' placeholder='nomor rekening' class='form-field'  type='text' required>
<br />
...
$nama_bank = $_POST[nama_bank];
  if (empty($_POST[nama_toko])){
   ...
   mysql_query("insert into toko_kosumen (id_konsumen,nama_toko,deskripsi,alamat_toko,nomo_rekening,nama_bank,status_toko) 
