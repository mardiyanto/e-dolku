...
<?php
    $edit = mysqli_query($koneksi, "SELECT * FROM orders, kustomer, kota, bank WHERE orders.id_kustomer=kustomer.id_kustomer AND kota.id_kota=kustomer.id_kota AND bank.id_bank=orders.id_bank_o AND id_orders='$_GET[id]'");
    ...
    if ($kn['status'] == 'Baru') {
        echo "<div class='cetak1'><a>Menunggu</a></div>";		
    } elseif ($kn['status'] == 'Berhasil') {
        echo "<div class='cetak2'><a>Berhasil</a></div>";		
    } else {
