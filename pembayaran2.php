...
if (empty($_SESSION['email']) || empty($_SESSION['pass'])) {
    ...
    $editkt = mysqli_query($koneksi, "SELECT * FROM orders, kustomer, kota WHERE kota.id_kota=orders.id_kota_o AND orders.id_kustomer=kustomer.id_kustomer AND kustomer.id_kustomer={$_SESSION['kustomer']}");
    $rk = mysqli_fetch_array($editkt);
    $editktp = mysqli_query($koneksi, "SELECT * FROM kustomer WHERE kustomer.id_kustomer={$_SESSION['kustomer']}");
...
<div class='bok4'>
    <form name='NamaForm' class="formoid-metro-cyan" style="background-color:#FFFFFF;font-size:14px;font-family:'Open Sans','Helvetica Neue','Helvetica',Arial,Verdana,sans-serif;color:#000000;" method="post" action="log.php?aksi=pembayaran">
        ...
        <div class="element">
            <label class="title">Metode Pengiriman Barang<span>*</span></label>
            <div class="large">
                <span>
                    ...
                    <i></i>Note : Jika memilih Kurir kami maka akan dikenakan biaya pengiriman. Jika memilih ambil sendiri berarti barang diambil di toko kami.
                </span>
            </div>
        </div>
        <div class="element">
            <label class="title">Metode Pembayaran<span>*</span></label>
            <div class="large">
                <span>
        ...
        <div class="element" id='DivTampil' style='display:none'>
            <label class="title">Transfer Ke Bank<span>*</span></label>
            <div class="large">
                <span>
                    <select name="bank">
                        <?php
...
<script language="JavaScript" type="text/JavaScript">
    $(function() {
        $('#provinsi').change(function() {
            $('#kota').html("<img src='ajax-loader.gif'>");
            var id = $(this).val();
    ...
    function DisplayShowHide() {
        if (document.NamaForm.pembayaran.value == "Bank Transfer") {
            document.getElementById('DivTampil').style.display = "";
        }
        if (document.NamaForm.pembayaran.value == "Bayar Ditempat") {
            document.getElementById('DivTampil').style.display = "none";
        }
    }
</script>
