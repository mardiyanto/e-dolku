...
<?php
    $edit = mysqli_query($koneksi, "SELECT * FROM orders
        INNER JOIN kustomer ON kustomer.id_kustomer = orders.id_kustomer
        INNER JOIN kota ON kota.id_kota = orders.id_kota_o
        INNER JOIN bank ON bank.id_bank = orders.id_bank_o
...
if ($kn['status'] == 'Baru') {
    ...
    echo "
        <div class='cetak'><a href='index.php?l=lihat&aksi=konfirmasi&id={$r['id_orders']}'>Konfirmasi Pembayaran</a></div>";
}
...
$sql2 = mysqli_query($koneksi, "SELECT * FROM orders_detail
                     INNER JOIN produk ON orders_detail.id_produk = produk.id_produk 
